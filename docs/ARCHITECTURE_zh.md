...
MCP 层注册了 6 个工具（`wolf_hunt`、`wolf_add`、`wolf_score`、`wolf_tailor`、`wolf_fill`、`wolf_reach`、`wolf_status`）。`wolf_add` 仅限 MCP，没有 CLI 对应命令——其调用方（AI agent）已经持有结构化内容，wolf 只需存储即可。输入输出 schema 见 [TYPES_zh.md § MCP Tool Schema](TYPES_zh.md#mcp-tool-schema)。
