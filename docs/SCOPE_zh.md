...
- **CLI**（commander.js）— 供终端用户使用
- **MCP 服务器**（MCP SDK）— 供 AI 代理使用（OpenClaw、LangGraph 等）
- 两个接口共享相同的核心逻辑，行为完全一致

### 外部编排

wolf 的设计理念是**被编排，而非自行编排**。外部工具（OpenClaw、n8n、LangGraph）可以通过 CLI（`--json` 输出）或 MCP 工具驱动 wolf。wolf 本身不管理工作流。
...
| **Cover Letter 排版 / PDF 导出** | wolf 生成 Markdown 格式的 cover letter 内容；排版和 PDF 转换交给外部工具 |
...
| **隐私** | API 密钥和 OAuth 凭证留在你的机器上（`~/.wolf/credentials/`、`.env`）；wolf 不会上传你的简历或个人数据，除非你主动发起的 API 调用 |
...
| 工具 | 关系 |
|---|---|
| **OpenClaw** | 上游编排者 — 可以在更大的代理工作流中调用 wolf 的 MCP 工具 |
| **n8n** | 可以通过 CLI shell 命令（`wolf hunt --json`）或 MCP 客户端连接来驱动 wolf |
| **LangGraph** | 可以通过 MCP 将 wolf 作为工具提供者，把每个命令当作图中的一个节点 |
| **Claude Desktop** | 可以连接到 wolf 的 MCP 服务器，在聊天界面中使用求职工具 |

wolf 始终是**被调用方**，而非调用方。它提供能力；编排逻辑在别处。
