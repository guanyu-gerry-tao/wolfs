...
- 通过 BrowserMCP 进行**浏览器辅助**提取（AI 驱动的页面导航）
...
- 存储在本地 `wolf.toml` — 除非你主动发起 API 调用，否则不会上传
- 供 `wolf fill`（表单填写）、`wolf reach`（邮件起草）和 `wolf tailor`（简历上下文）使用
...
| **网络** | 仅在调用外部 API（Claude、Gmail 以及你配置的 provider API）时需要网络；核心的追踪和状态功能可离线使用 |
